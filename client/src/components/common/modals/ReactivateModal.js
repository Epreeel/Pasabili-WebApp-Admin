...
export default function ReactivateModal(props) {
  const { refetch: employeeRefetch } = useEmployeePageContext();
  const { refetch: customerRefetch } = useCustomerPageContext();
  const refetch = props.module === 'employees' ? employeeRefetch : customerRefetch;
